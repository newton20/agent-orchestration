...
- **The SessionStart hook MUST do non-destructive token filtering BEFORE the `.consuming-*` atomic rename** (the existing destructive consume step from Unit 5). The hook iterates fresh `.pending-*` candidates, reads each one's token via `fs.readFileSync` (peek, no rename), and skips any candidate whose file-token does not match the tab-bound token from argv/env. Only after a token-matching candidate is identified does the hook proceed to the `.consuming-*` rename.
- **After the rename, re-validate the token.** A race between the peek and the rename could swap the file content (extremely unlikely, but the existing protocol assumes atomic-rename guarantees only on the rename itself). The hook reads the post-rename file's token and bails if it no longer matches the tab-bound token (renaming back is unnecessary; the file is now `.consuming-*` and orphan to whichever tab thought it was renaming for itself).
- An orphan tab carries the token from its original (now timed-out) spawn. When the hook fires and scans fresh `.pending-*` files, it filters out any whose file-token does not match the orphan's argv-token. If the only fresh flag is the orchestrator's NEW per-spawn flag (carrying a different token), the orphan's filter pass yields zero candidates → no destructive rename → fresh flag remains for the intended new tab to consume.

**Critical:** comparing tokens read only from the flag file is NOT sufficient — the hook would just read the fresh-tick token and accept it (the bug). The token must be bound to the spawned tab out-of-band so it cannot be replaced by reading a different file. **Equally critical:** the token check must happen BEFORE the destructive `.consuming-*` rename, not after — otherwise the orphan removes the fresh flag and then refuses to deliver it, leaving the intended new tab with no prompt to consume.
...
- [ ] Test specifically covers the orphan-grabs-fresh-flag case: orphan tab's argv-token is OLD (from its original timed-out spawn), the fresh next-tick `.pending-*` file's token is NEW, the hook's pre-rename token filter detects the mismatch, the file is NOT renamed to `.consuming-*`, and the fresh flag remains intact for the intended new tab to consume.
- [ ] Test specifically covers the **destructive-rename ordering**: the SessionStart hook MUST filter by token BEFORE the `.consuming-*` rename. A test where the orphan's hook is artificially induced to rename-then-check would FAIL (the fresh flag would be consumed and then refused, leaving the intended tab with no prompt). The implementation must do peek-then-rename, not rename-then-peek.
...
- [ ] Test for post-rename revalidation: simulate a race where the file content swaps between peek and rename → hook bails on revalidate, doesn't deliver an unrelated prompt.
